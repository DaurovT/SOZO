 ...
 *
 * Это не «когда-нибудь потом», а условие запуска. На нынешних снимках чужие
 * люди в форме чужих компаний: на `ilhom.webp` человек в рубашке с логотипом
 * «MahadevTravels», рядом мотоцикл Hero Honda. Подписать такой кадр «Ильхом,
 * сантехник, Юнусабад» — значит выдать сотрудника чужой фирмы за своего
 * мастера на странице, которая ровно про то, что мы знаем, кто к вам едет.
 * Кадры к тому же сняты по-разному: анфас и профиль, тёплый и холодный свет,
 * разное кадрирование. `object-position` в styles.css сглаживает это лишь
 * отчасти — выравнивает пересъёмка.
 *
