  ...
  async deliver(input: DeliverInput): Promise<DeliveryRec> {
    ...
    for (const device of devices) {
      ...
      if (res.gone) {
        ...
        // будет отнимать запрос при каждой рассылке до скончания века
        this.devices.markGone(device.token);
        // Причина называется своим именем: «не доставлено» и «устройства
        // больше нет» чинятся по-разному — первое ждёт повтора, второе
        // ждёт, когда человек откроет приложение
        lastError = 'token_gone';
        continue;
      }
      lastError = res.error;
