...
export class PublicBuildingController {
  ...
  /**
   * Проверка пропуска охраной (DEV-15 §11.3).
   *
   * Без авторизации: у охранника на посту нет учётной записи, а требовать её
   * значит получить пост, который проверяет пропуска на глаз. Принимается и
   * QR-токен, и числовой код — код диктуют голосом, когда камера не читает.
   *
   * Ответ узкий намеренно: кого ждут, куда и до какого времени. Телефон
   * жителя охране не нужен, а лишнее поле в таком ответе живёт годами.
   */
  @Get('passes/:token')
  checkPass(@Param('token') token: string) {
    return this.access.checkPass('t0', token);
  }
