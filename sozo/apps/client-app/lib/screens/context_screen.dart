...
class _ContextGateState extends State<ContextGate> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // Офлайн на старте — не повод не пускать в приложение, **если профиль
      // есть в кеше**. Раньше условие было безусловным, а кеша не было вовсе:
      // оболочка строилась с пустым `me`, и это выглядело как потеря данных,
      // а не как отсутствие сети. Теперь пускаем только на кеше, а без него
      // честно показываем, что случилось, и даём «Повторить»
      if (!e.isOffline || session.me == null) {
